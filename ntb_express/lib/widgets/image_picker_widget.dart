...
class _ImagePickerWidgetState extends State<ImagePickerWidget> {
    ...
    for (Asset a in files) {
      ...
      final fileHolder = FileHolder(
          file: await compute(_computeFile as ComputeCallback<FileHolderParser, File>,
              FileHolderParser(fileByteData: byteData, fileName: a.name)), key: '', fileUrl: '');
      int index = -1;
  ...
  Future<void> _takePhoto() async {
    ...
    if (mounted) {
      ...
      setState(() => _files.add({} as FileHolder));

      final fileHolder = FileHolder(
          file: await compute(
              _computeFile as ComputeCallback<FileHolderParser, File>,
              FileHolderParser(
                  file: file,
                  fileName:
                      file.path.substring(file.path.lastIndexOf('/') + 1))), key: '', fileUrl: '');
  ...
  static Future<File?> _computeFile(FileHolderParser parser) async {
    ...
    return await Utils.resizeImage(MemoryFileSystem().file(parser.fileName)
      ..writeAsBytesSync((parser.file == null
          ...
          : parser.file?.readAsBytesSync()) as List<int>));
  }
